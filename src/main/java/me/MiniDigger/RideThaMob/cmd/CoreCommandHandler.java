 ...
 */
package me.MiniDigger.RideThaMob.cmd;
...
import me.MiniDigger.RideThaMob.cmd.api.BukkitCommand;
import me.MiniDigger.RideThaMob.cmd.api.BukkitCompleter;
...
import me.MiniDigger.RideThaMob.cmd.api.Completer;
import me.MiniDigger.RideThaMob.cmd.util.MultiMap;
import me.MiniDigger.RideThaMob.lang.LangHandler;
...
public class CoreCommandHandler implements CommandHandler {
	...
	public CoreCommandHandler(final Map<String, String> relocations) {
		...
		if (plugin.getServer().getPluginManager() instanceof SimplePluginManager) {
			...
			try {
				...
				// field2.setAccessible(true);
				// final Map<String, org.bukkit.command.Command>
				// newknownCommands = new HashMap<>();
				// @SuppressWarnings("unchecked")
				// final Map<String, org.bukkit.command.Command> knownCommands =
				// (Map<String, org.bukkit.command.Command>) field2
				// .get(map);
				// for (final Map.Entry<String, org.bukkit.command.Command>
				// entry : knownCommands.entrySet()) {
				// for (final String key : relocations.keySet()) {
				// if (entry.getKey().startsWith(key)) {
				// newknownCommands.put(entry.getKey().replaceFirst(key,
				// relocations.get(key)),
				// entry.getValue());
				// Core.getCore().getInstance().info("Relocation: Moving " +
				// entry.getKey() + " to "
	...
	@Override
	public boolean handleCommand(final CommandSender sender, final String label, final org.bukkit.command.Command cmd,
			String[] args) {
		for (int i = args.length; i >= 0; i--) {
			...
			if (commandMap.containsKey(cmdLabel)) {
				...
				final CommandArgs cmdArgs = new CoreCommandArgs(sender, cmd, label, args,
						cmdLabel.split("\\.").length - 1);

				if (sender instanceof Player) {
					final Player user = (Player) sender;
					if (!user.hasPermission(command.permission())) {
						final FancyMessage msg = LangHandler.getInstance().getPrefix().then(command.noPerm())
				...
				if (!command.consol() && !(sender instanceof Player)) {
					final FancyMessage msg = LangHandler.getInstance().getPrefix().then(command.noConsol())
							.color(ChatColor.DARK_RED);
					msg.send(sender);
					return true;
				}
				...
				if (cmdArgs.getArgs().length < command.min()) {
					final FancyMessage msg = LangHandler.getInstance().getPrefix().then(command.fewArgs())
							.color(ChatColor.DARK_RED);
					msg.send(sender);
					return true;
				}
				...
				if (cmdArgs.getArgs().length > command.max() && command.max() != -1) {
					final FancyMessage msg = LangHandler.getInstance().getPrefix().then(command.manyArgs())
							.color(ChatColor.DARK_RED);
					msg.send(sender);
					return true;
				}
	...
	public void registerHelp() {
		...
		final IndexHelpTopic topic = new IndexHelpTopic(plugin.getName(), "All commands for " + plugin.getName(), null,
				help, "Below is a list of all " + plugin.getName() + " commands:");
